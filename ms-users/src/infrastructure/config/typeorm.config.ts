...
export class TypeOrmConfigService implements TypeOrmOptionsFactory {
  ...
  public createTypeOrmOptions(): TypeOrmModuleOptions {
    const typeOrmOptions = {
      name: this._configService.get<string>('DATABASE_MS_USERS_NAME', undefined),
      ...
      database: this._configService.get<string>('DATABASE_MS_USERS_NAME', undefined),
      debug: BooleanUtils.strToBoolWithDefault(
        this._configService.get<boolean>('DATABASE_MS_USERS_DEBUG', undefined),
        undefined,
      ),
      logging: BooleanUtils.strToBoolWithDefault(
        this._configService.get<boolean>('DATABASE_MS_USERS_LOGGING', undefined),
        undefined,
      ),
    ...
    if (typeOrmOptions.name === undefined) {
      throw new Error('Name database is undefined on ms-users!')
    }

    if (typeOrmOptions.type === undefined) {
      throw new Error("Environment variable 'DATABASE_DEFAULT_TYPE' cannot be undefined on ms-users!")
    }

    if (typeOrmOptions.host === undefined) {
      throw new Error("Environment variable 'DATABASE_DEFAULT_HOST' cannot be undefined on ms-users!")
    }

    if (isNaN(typeOrmOptions.port)) {
      throw new Error("Environment variable 'DATABASE_DEFAULT_PORT' cannot be undefined on ms-users!")
    }

    if (typeOrmOptions.username === undefined) {
      throw new Error("Environment variable 'DATABASE_DEFAULT_USERNAME' cannot be undefined on ms-users!")
    }

    if (typeOrmOptions.password === undefined) {
      throw new Error("Environment variable 'DATABASE_DEFAULT_PASSWORD' cannot be undefined on ms-users!")
    }

    if (typeOrmOptions.database === undefined) {
      throw new Error("Environment variable 'DATABASE_MS_USERS_NAME' cannot be undefined on ms-users!")
    }

    if (typeOrmOptions.debug === undefined) {
      throw new Error("Environment variable 'DATABASE_MS_USERS_DEBUG' cannot be undefined on ms-users!")
    }

    if (typeOrmOptions.logging === undefined) {
      throw new Error("Environment variable 'DATABASE_MS_USERS_LOGGING' cannot be undefined on ms-users!")
    }
